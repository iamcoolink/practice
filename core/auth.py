...
class AuthHandler(metaclass=SingletonMeta):
    ...
    def verify_access_token(self, token: str) -> int:
        ...
        try:
            payload = jwt.decode(token, self.secret, algorithms=["HS256"])
            if payload.get("sub") != str(TokenTypeEnum.ACCESS_TOKEN.value):
                raise HTTPException(status_code=HTTP_403_FORBIDDEN, detail="Access Token is invalid")
            user_id = payload.get("iss")
            if user_id is None:
                raise HTTPException(status_code=HTTP_403_FORBIDDEN, detail="Token missing user ID")
            # iss 可能是字符串数字，也可能是 JSON 字符串
            try:
                return int(user_id)
            except ValueError:
                import json
                ...
                if isinstance(data, dict) and "id" in data:
                    ...
                    raise HTTPException(status_code=HTTP_403_FORBIDDEN, detail="Invalid user ID format")
        except jwt.ExpiredSignatureError:
